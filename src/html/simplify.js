...
function removeEmpty(nodes) {
    return flatMap(nodes, function(node) {
        var val = emptiers[node.type](node);    // Типо тут element или text
        return val;
    });
}
...
function elementEmptier(element) {
    ...
    if (children.length === 0 && !ast.isVoidElement(element)) {
        ...
        // NOTE: модифицировано, ибо это просто вырезает инфомрацию из элемента, оставляя только тег, детей и немного ещё чего
