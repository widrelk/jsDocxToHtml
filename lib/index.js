...
var elementToHtmlConverters = {
    ...
    "hyperlink": function(hyperLink) {
        var result = `<a href=\"${hyperLink.href}\">` + convertElementsToHtml(hyperLink.children) +"<a>";
        return(result)
    }
}
...
function convertParagraphToHtml(paragraph) {
    ...
    if (paragraph.indent.left != 0) {
        result += `padding-left: ${paragraph.indent.left}; `;
    }
    if (paragraph.indent.right != 0) {
        result += `padding-right: ${paragraph.indent.right}; `;
    }

    if (paragraph.indent.hanging != 0) {
        result += `text-indent: ${-1 * paragraph.indent.hanging}; `;
    }
    if (paragraph.indent.firstLine != 0) {                                // Done this way because if both specified firstLine is in control
        result += `text-indent: ${paragraph.indent.firstLine}; `;
    }

    if (paragraph.spacing.before != 0) {
        result += `margin-top: ${paragraph.spacing.before}; `;
    }
    if (paragraph.spacing.after != 0) {
        result += `margin-bottom: ${paragraph.spacing.after}; `;
    }

    if (paragraph.spacing.line != 0) {
        result += `line-height: ${paragraph.spacing.line}; `;
    }
    ...
    if (rPr) {
        ...
        var font = `font: ${rPr.fontSize}pt \"${rPr.font}\" `;
        if (rPr.isItalic) {
            font += "italic ";
        }
        if (rPr.isBold) {
...
function convertRunToHtml(run) {
    ...
    var font = `font: ${run.fontSize}pt \"${run.font}\" `;
    if (run.isItalic) {
        font += "italic ";
    }
    if (run.isBold) {
...
function convertTable(table) {
    ...
    result += table.children.map(function(tableRow) {
        ...
        row += tableRow.children.map(function(tableCell) {
            var cell = `<td rowspan=\"${tableCell.rowSpan}\" colspan=\"${tableCell.colSpan}\">`;
