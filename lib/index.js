    ...
    return unzip.openZip(input)
        ...
        .then(function(docxFile) {
            docxReader.readStylesFromZipFile(docxFile, "/word/styles.xml").then(function(styles){   // Считываем стили
                ...
                //    // Назначение не ясно, эта штука ссылается на функцию, которая просто возвращает аргумент
                //    return documentResult.map(options.transformDocument);
                //})
                .then(function(documentResult) {
                    documentResult.value.children.map(function (paragraph) {
                        debugger;

                        if (paragraph.styleId){                                             // Если у paragraph есть id
                            var style = styles.findParagraphStyleById(paragraph.styleId);
                            debugger;
                        }
                        // Смотрим для детей
                        paragraph.children.map(function (child) {
                            if (child.styleId){                                          // если у ребёнка есть id
                                if (child.type == "run"){                                      
