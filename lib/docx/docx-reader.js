...
function read(docxFile, input) {
    ...
    return promises.props({
        ...
        return readXmlFileWithBody(result.partPaths.mainDocument, result, function(bodyReader, xml) {
            // НА ЭТОМ ЭТАПЕ ЕСТЬ ПРОСТО СЧИТАННЫЙ XML ИЗ DOCUMENT.XML
            // Смысл во flatMap не ясен от слова совсем
            return result.notes.flatMap(function(notes) {
                return result.comments.flatMap(function(comments) {
                    ...
                    return new Result({xmlResult: reader.convertXmlToDocument(xml).value.children, footNotes: result.footNotes, comments: result.comments.value, headers: result.headers, footers: result.footers}, {});
                });
            });
        });
    });
