...
function BodyReader(options) {
    ...
    function readParagraphProperties(element) {
        ...
        //return readParagraphStyle(element).map(function(style) {
            ...
            sName = style["styleName"];

            var runPr = readRunProperties(findTagByNameInArray(element.children, "w:rPr"));
            if (runPr) {
                runPr = runPr.value;
            }
            ...
            if (spcng) {
                ...
                spcng = style.pPr.spacing;                                                          // Every style will contain at least a default value
            }

            var parPr = {
                alignment: readChildTagVal(element, "w:jc"),
                numbering: readNumberingProperties(element.firstOrEmpty("w:numPr"), numbering),
                indent: readParagraphIndent(findTagByNameInArray(element.children,"w:ind")),
                spacing: spcng,
                rPr: runPr,
                ...
                outlineLvl: style.pPr.outlineLvl,
                /** Specifies that all lines of the paragraph are to be kept on a single page when possible.*/
                keepLines: readBooleanElement(findTagByNameInArray(element.children, "w:keeplines")),
                ...
                keepNext: readBooleanElement(findTagByNameInArray(element.children, "w:keepnext")),
            };
    ...
    function readRunProperties(element) {
        try {
            if (element){
                ...
                result = style.rPr;  
                result["styleId"] = sId;                                                           // Using the obtained style as the basis before applying
    ...
    var xmlElementReaders = {
        ...
        "w:r": function(element) {
            //var val1 = readXmlElements(element.children);
            // TODO: посмотреть что с hyperlink
            // Что-то где-то поломалось, больше не работает
            return readXmlElements(element.children)
                .map(function(child) {
                try {
        ...
        // TODO: сделать чтение этого тега
        //"w:sectPr":
    };
    return {
