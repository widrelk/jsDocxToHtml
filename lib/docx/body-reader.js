...
function BodyReader(options) {
    ...
    var xmlElementReaders = {
        ...
        "w:drawing": function(element) {
            ...
            return elementResult({
                type: "image",
                id: readPicId(element),
                val: val.read().then(function(res) {
                        sessionStorage.setItem(this.id, URL.createObjectURL( new Blob([res.buffer], { type: 'image/png'}) ))
                    })
            })
    ...
    function readTableRow(element) {
        ...
        cells.map(function(cell) {
            cell["cellStyle"] = element.cellStyle
        })
    ...
    function readTableCell(element) {
        return readXmlElements(element.children).map(function(children) {
            ...
            // TODO: сделать смешивание properties
            ...
            cell["cellProps"] = readTableCellProperties(properties)
            return cell;
        });
    }
