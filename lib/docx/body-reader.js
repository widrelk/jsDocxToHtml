...
// TODO: может вставить чтение всех полей тут?
//  по идее, у элемента может быть стиль из styles.xml и поверх него накладываться ещё что-то
function BodyReader(options) {
    ...
    // Reads w:rPr tag content
    function readRunProperties(element) {
        return readRunStyle(element).map(function(style) {   // Зачем тут map - не понятно
            // TODO: попробовать переделать как в rPP
            sId = null;
            sName = null;
            if (style) {
                sId = style.styleId;
            ...
            return {
                ...
                underlineColor: element.firstOrEmpty("w:u"), 
    ...
    function readParagraphProperties(element) {
        //return readParagraphStyle(element).map(function(style) {
            style = styles.findParagraphStyleById(element.styleId);
            if (!style) {
                style = styles.findParagraphStyleById("a");                // a is a default paragraph style id
            }
            sId = style.styleId;
            sName = style.Name;

            var runPr = readRunProperties(element.firstOrEmpty("rPr"));
            var parPr = {
            ...
            var spcng = element.firstOrEmpty("w:spacing");
            if (spcng) {
                spcng = {
                    after: spcng.firstOrEmpty("w:after"),
                    before: spcng.firstOrEmpty("w:before"),
            ...
            return {
                ...
                outlineLvl: element.firstOrEmpty("w:outlineLvl"),
                /** Specifies that all lines of the paragraph are to be kept on a single page when possible.*/
                keepLines: element.readBooleanElement("w:keeplines"),
                ...
                keepNext: element.readBooleanElement("w:keepNext")
            };
        //});
    }
    ...
    // TODO: Все модификации чтения - здесь
