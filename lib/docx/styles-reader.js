...
function readTableCellProperties(tcPr) {
    ...
    */  
    ...
    // TODO: тут могут быть проценты, внезапно. Надо подобрать тест и сделать   
    var tcW = (findTagByNameInArray(tсPr.children, "w:tcW") || {attributes: {"w:w": 0}}).attributes["w:w"] / 20
...
function readBordersTag(borders) {
    ...
    var res = {};
        tcBorders.children.forEach(function(child){
            res[child.name.split(':')[1]] = {           // Получаем название границы
                style: child.attributes["w:val"],
                width: child.attributes["w:sz"] || 0 / 8,   // See sz http://officeopenxml.com/WPtableCellProperties-Borders.php
