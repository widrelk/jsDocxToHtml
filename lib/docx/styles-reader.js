...
//const { element } = require("../styles/html-paths");
    ...
    return {
        findParagraphStyleById: function(styleId) {
            if (paragraphStyles[styleId]){
                return JSON.parse(JSON.stringify(paragraphStyles[styleId]));
            } else {
                return(null);
            }
        },
...
try {
    if (element){
        ...
        sName = element.firstOrEmpty("w:name");
        if (sName) {
            sName = sName.attributes["w:val"];
        }
        ...
        // Done this way so we can avoid null font. Using "default" if font is not stated in style
        // TODO: Иногда указывается только семейство шрифтов. Что в этом сучае делать - не особо понятно, пока что стоит default
        ...
        return {
            ...
            styleId:            sId,
            styleName:          sName,
            isBold:             readBooleanElement(findTagByNameInArray(element.children, "w:b")),
            isItalic:           readBooleanElement(findTagByNameInArray(element.children, "w:i")),
            isStrikethrough:    readBooleanElement(findTagByNameInArray(element.children, "w:strike")),
            /** Specifies the underline pattern if one is used (single/thick/dash...) */
            underline:          readChildTagVal(element, "w:u"),
            underlineColor:     readChildTagAttr(element, "w:u", "w:color"),
            font:               font,                                                   // TODO: сделать проддержку других семей шрифтов
            /** Specifies font size in standart points */
            fontSize:           fontSize,
            color:              readChildTagVal(element, "w:color"),
            highlight:          readChildTagVal(element, "w:highlight"),
            verticalAlignment:  readChildTagVal(element, "w:vertAlign"),
            isDStrikethrough:   readBooleanElement(findTagByNameInArray(element.children, "w:dstrike")),
            isCaps:             readBooleanElement(findTagByNameInArray(element.children, "w:caps")),
            isSmallCaps:        readBooleanElement(findTagByNameInArray(element.children, "w:smallcaps"))
        };
    } else {
        return {
                type: "runProperties",
...
function readStyleElement(styleElement) {
    ...
    if (type == "paragraph"){
        ...
        if (pPr) {
            ...
            if (spcng) {
                spcng = { 
                            after: spcng.attributes["w:after"],             // Distance between next p
                            before: spcng.attributes["w:before"],          // Distance between previous p
                            line: spcng.attributes["w:line"]                // Line height / distance between lines
                        }        
            } else {
                spcng = { 
                            before: "160pt",                                  // Default style values
                            after: "0pt",
                            line: "0pt"
                        }
            }
            ...
            result["pPr"] = {
                alignment: readChildTagVal(styleElement, "w:jc"),
                //numbering: readNumberingProperties(element.firstOrEmpty("w:numPr"), numbering),   TODO: разобрать numbering
                indent: readParagraphIndent(findTagByNameInArray(styleElement.children,"w:ind")),
                spacing: spcng,
                rPr: runPr,
                link: rLink,
                outlineLvl: readChildTagAttrVal(styleElement, "w:outlineLvl"),
                keepLines: readBooleanElement(findTagByNameInArray(styleElement.children,"w:keeplines")),
                keepNext: readBooleanElement(findTagByNameInArray(styleElement.children,"w:keepnext"))
            };
        } else {
            result["pPr"] = {                                               // Like default paragraph properties
                ...
                //numbering: null,
                indent: null,
                spacing: {before:"120", after:"0", line:"0"},               // TODO: Проверить дефолтный spacing
 ...
 * @param {*} tag 
 * @param {*} attribute 
 * @returns returns tag's attribute val if it exists, or false
 */
...
function readParagraphIndent(element) {
    ...
    // in Word everything is measured in halfpoints, so we need to halve the value just like as with font size
