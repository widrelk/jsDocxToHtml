...
function readTableStyleProperties(tblPr) {
    ...
    */
    ...
    var tblBorders = findTagByNameInArray(tblPr.children, "w:tblBorders");
    if (tblBorders) {
        var res = {};
        tblBorders.children.forEach(function(child){
            res[child.name.split(':')[1]] = {
                style: child.attributes["w:val"],
                width: child.attributes["w:sz"],
    ...
    var tblCellMar = findTagByNameInArray(tblPr.children, "w:tblCellMar");
    if (tblCellMar) {
        tblCellMar = {
                                // Если результат fTBNIA будет null, используется foo и не выбросится ошибка с undefined
            top:    (findTagByNameInArray(tblCellMar.children, "w:top") || {attributes: {name: "foo"}}).attributes["w:w"] / 20,
            left:   (findTagByNameInArray(tblCellMar.children, "w:left") || {attributes: {name: "foo"}}).attributes["w:w"] / 20,
            bottom: (findTagByNameInArray(tblCellMar.children, "w:bottom") || {attributes: {name: "foo"}}).attributes["w:w"] / 20,
            right:  (findTagByNameInArray(tblCellMar.children, "w:right") || {attributes: {name: "foo"}}).attributes["w:w"] / 20
        }
    }
    
    var tblCellSpacing = findTagByNameInArray(tblPr.children, "w:tblCellSpacing");
    if (tblCellSpacing) {
        debugger;       // Аналогично http://officeopenxml.com/WPtableCellSpacing.php
    }
       
    var tableWidth = (findTagByNameInArray(tblPr.children, "w:tblW") || {attributes: {name: "foo"}}).attributes["w:w"]
